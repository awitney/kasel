...
use lib 'kasel/kasel/workflow_pairs/scripts';
...
my ( $qual_cutoff, $dp4_cutoff, $dp_cutoff, $dpmax_cutoff, $af_cutoff, $mq_cutoff, @vcfs, $chrom, @vcfdirs,
		$exclude, $include, $chromv, $vcflist,
...
if ( $help || !(@vcfs || @vcfdirs || $vcflist || $include) || !$chrom || ( $phylip && !$noindels ) ) { pod2usage(1) }
...
if ( $exclude ) {
	print  "Loading isolate files to exclude from ".$exclude."\n" if $verbose;
...
#		print STDERR "EXCLUDED LIST: ".$isolate."\n";
...
if ( $include ) {
	print  "Loading isolate files to include from ".$include."\n" if $verbose;
	open(INCLUDE, $include) || die "\nCannot open $include: $!\n";
...
print "Isolates to be included ".(keys(%$included))."\n" if $verbose;
...
if ( $vcflist ) {
	print  "Loading VCF files listed in ".$vcflist."\n" if $verbose;
	open(VCFLIST, $vcflist) || die "\nCannot open $vcflist: $!\n";
	...
	while (<VCFLIST>) {
		...
		my ($label, $file);
		if ( $line =~  m/(.+),(.+${chrom}_.+.all.vcf.gz)/ ) {
			$label = $1;
			$file  = $2;
		} elsif ( $line =~ m/.+${chrom}_(.+).all.vcf.gz/ ) {
			$label = $1;
			$file  = $line;
		} else {
		...
		print "-".$line."-".$label."-".$file."-\n";
		push(@vcfs, $label.",".$file);
	}

	close(VCFLIST);
}

print "VCFs to be included ".(@vcfs)."\n" if $verbose;
...
if ( @vcfdirs ) {
	for my $dir ( @vcfdirs ) {
		...
		my @files = grep { /$chrom/ && (/.all.vcf.gz$/ || /.all.vcf$/) && -f "$dir/$_" } readdir($dh);
		
		foreach my $file ( @files ) {
			my ($ref, $label) = ( $file =~ m/(NC_.+?)_(.+).all.vcf[.gz]*$/ );
			
			print "Found: ".$file."\t".$ref." -- ".$label."\t".$label.",".$dir."/".$file."\n";
			...
			if ( keys %$included && ! $included->{$label} ) {
				next;
			}
			
			print $file."\t".$ref." -- ".$label."\t".$label.",".$dir."/".$file."\n";
			
			push(@vcfs, $label.",".$dir."/".$file);		
		}
		closedir $dh;
	}
}

print "VCF files included ".@vcfs."\n" if $verbose;
print "VCF files excluded ".$excluded_count."\n" if $verbose;

## Process vcfs

# add version to chrom name if exists
$chrom .= '.'.$chromv if $chromv;
...
foreach my $vcf ( @vcfs ) {
	...
	if ( ! $label || ! $file ) {
		...
		($label) = $vcf =~ m/.+\/(.+).all.vcf.gz/;

		$file = $vcf;

		warn "LABEL FOUND: ".$vcf." -- ".$label."\n";
	}
...
my $fconst_outfile = $outdir.'/'.$phylip.'.fconst.txt';
open (FCONST, ">".$fconst_outfile) || die "\nCannot open $fconst_outfile: $!\n";
...
#			}
	...
	print  "\t".$vcffile."\tUncertain sites = ".$uncertain_total." / ".$total."\tVariant sites = ".$variant_total."\tvcf_data = ".join(", ", map { $_." = ".(keys %{$vcf_data->{$_}}) } sort keys %{$vcf_data})."\n";
...
print "Missing sites: ".(keys %{$non_core})."\n" if $verbose;
...
my $counter1;
die "No data left, somethimg went wrong!\n" unless $combined_data->{$chrom};
...
#print "DD: ".$vcf."\t".$i."\t".$alt."\n";
			...
			# sets alt to equal first base call, to be compared with all other in that column
...
#print "EE: ".$i."\t".$alt."\n";
	...
	my $fconst_A = defined $core_sites->{'A'} ? $core_sites->{'A'} : 0;
	my $fconst_C = defined $core_sites->{'C'} ? $core_sites->{'C'} : 0;
	my $fconst_G = defined $core_sites->{'G'} ? $core_sites->{'G'} : 0;
	my $fconst_T = defined $core_sites->{'T'} ? $core_sites->{'T'} : 0;

	print "-fconst ".$core_sites->{'A'}.','.$core_sites->{'C'}.','.$core_sites->{'G'}.','.$core_sites->{'T'}."\n";
	print FCONST "-fconst ".$fconst_A.','.$fconst_C.','.$fconst_G.','.$fconst_T."\n";
